# WiFi Capabilities

## 2.4 GHz Wi-Fi Capabilities / Properties

| Parameter | Observation |
|---|---|
| Interface Name | Wi-Fi |
| Wi-Fi Adapter | MediaTek Wi-Fi 6 MT7921 Wireless LAN Card |
| Vendor | MediaTek, Inc. |
| Driver Version | 3.3.0.901 |
| Supported Standards | 802.11b, 802.11g, 802.11n, 802.11a, 802.11ac, 802.11ax |
| Connected SSID | Goutham's realme GT 6T |
| Frequency Band | 2.4 GHz |
| Channel | 1 |
| Radio Type | 802.11n |
| Network Type | Infrastructure |
| Authentication | WPA2-Personal |
| Cipher | CCMP |
| Receive Rate | 144.4 Mbps |
| Transmit Rate | 144.4 Mbps |
| Signal Strength | 95% |
| RSSI | -34 dBm |
| Number of Supported Bands | 2 (2.4 GHz and 5 GHz) |
| WPA3 Support | Yes |
| 802.11w Management Frame Protection | Supported |
| Wireless Display Support | Yes |
| Hosted Network Support | No |
| FIPS 140 Mode Support | Yes |
<img width="1143" height="1008" alt="image" src="https://github.com/user-attachments/assets/0073fcec-c1e4-4cc3-a5c1-24af0127f488" />
<img width="1036" height="698" alt="image" src="https://github.com/user-attachments/assets/49f3406c-468a-4da7-8d29-3f5575cafb7d" />

## 5 GHz Wi-Fi Capabilities / Properties  

| Parameter | Observation |
|---|---|
| Interface Name | Wi-Fi |
| Wi-Fi Adapter | MediaTek Wi-Fi 6 MT7921 Wireless LAN Card |
| Vendor | MediaTek, Inc. |
| Driver Version | 3.3.0.901 |
| Supported Standards | 802.11b, 802.11g, 802.11n, 802.11a, 802.11ac, 802.11ax |
| Connected SSID | Goutham's realme GT 6T |
| Frequency Band | 5 GHz |
| Channel | 149 |
| Radio Type | 802.11ac |
| Network Type | Infrastructure |
| Authentication | WPA2-Personal |
| Cipher | CCMP |
| Receive Rate | 866.7 Mbps |
| Transmit Rate | 866.7 Mbps |
| Signal Strength | 84% |
| RSSI | -58 dBm |
| Number of Supported Bands | 2 (2.4 GHz and 5 GHz) |
| WPA3 Support | Yes |
| 802.11w Management Frame Protection | Supported |
| Wireless Display Support | Yes |
| Hosted Network Support | No |
| FIPS 140 Mode Support | Yes |
<img width="1187" height="998" alt="image" src="https://github.com/user-attachments/assets/702c52d2-e5fa-4e8b-95ca-4104080fc876" />
<img width="953" height="680" alt="image" src="https://github.com/user-attachments/assets/fec64f33-4b81-4568-be08-1a045429d088" />

## Differences Between 2.4 GHz and 5 GHz Wi-Fi Capabilities

| Parameter | 2.4 GHz | 5 GHz |
|---|---|---|
| Frequency Band | 2.4 GHz | 5 GHz |
| Channel Used | 1 | 149 |
| Radio Type | 802.11n | 802.11ac |
| Receive Rate | 144.4 Mbps | 866.7 Mbps |
| Transmit Rate | 144.4 Mbps | 866.7 Mbps |
| Signal Strength | 95% | 84% |
| RSSI | -34 dBm | -58 dBm |
| Speed | Moderate | Very High |
| Coverage Range | Longer | Shorter |
| Wall Penetration | Better | Weaker |
| Interference | Higher (more crowded band) | Lower |
| Suitable Applications | Browsing, IoT, long-range connectivity | HD streaming, gaming, high-speed transfer |
| Number of Non-overlapping Channels | Fewer | More |
| Congestion | High | Low |
| Latency | Higher | Lower |
| Stability at Long Distance | Better | Reduces faster with distance |
| Power Consumption | Lower | Slightly Higher |
| Typical Usage | General home coverage | High-performance networking |
