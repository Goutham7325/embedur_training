...
DHCP Client → DHCP Discover (Broadcast) → DHCP Server  
DHCP Server → DHCP Offer (IP, Gateway, Lease) → DHCP Client  
DHCP Client → DHCP Request (Accept Offer) → DHCP Server  
DHCP Server → DHCP ACK (Confirm Assignment) → DHCP Client
